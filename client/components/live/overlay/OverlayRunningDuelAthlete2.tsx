    ...
    return (
        ...
        >
            ...
            <Fade in={showTieBreak}>
                ...
                >
                    <Typography variant="h4">
                        <u>{`Score ${data.measurements?.length}`}</u>
                    </Typography>
                    <Typography variant="h4">
                        {`${
                            data?.measurements &&
                            (data.measurements.at(-1)?.value
                                ? data.measurements.at(-1)?.method == "time"
                                    ? toReadableTime(
                                          data.measurements[
                                              data.measurements?.length - 1
                                          ].value
                                      )
                                    : data.measurements.at(-1)?.id == 2
                                    ? data.measurements.at(-1)?.value + " reps"
                                    : "CAP+ " + data.measurements.at(-1)?.value
                                : "DNF"
                                ? toReadableTime(
                                      data?.measurements.at(-1)?.value || 0
                                  )
                                : "DNF")
                        }`}
                    </Typography>
